...
public class ISW2SchemaC3LLMTest {
  ...
  public void primitiveFactoriesExposeTypeNameAndNullability() {
    for (Type type : Arrays.asList(Type.STRING, Type.BYTES, Type.INT, Type.LONG, Type.FLOAT, Type.DOUBLE, Type.BOOLEAN,
        Type.NULL)) {
  ...
  public void unionPreservesBranchOrderAndSupportsNameLookup() {
    ...
    Schema union = Schema.createUnion(Schema.create(Type.NULL), named, Schema.create(Type.STRING));

    assertTrue(union.isUnion());
    assertTrue(union.isNullable());
    assertEquals(Arrays.asList(Type.NULL, Type.FIXED, Type.STRING), Arrays.asList(union.getTypes().get(0).getType(),
  ...
  public void enumExposesSymbolsOrdinalsMembershipAndDefault() {
    Schema schema = Schema.createEnum("Status", "state", "example", Arrays.asList("NEW", "DONE"), "NEW");
  ...
  public void recordAssignsFieldPositionsAndSupportsLookup() {
    ...
    Field active = new Field("active", Schema.create(Type.BOOLEAN), null, true, Field.Order.DESCENDING);
    Schema record = Schema.createRecord("User", "user doc", "example", false, Arrays.asList(id, active));
  ...
  public void recordRejectsDuplicateFieldNames() {
    ...
    try {
      record.setFields(
          Arrays.asList(new Field("value", Schema.create(Type.INT)), new Field("value", Schema.create(Type.STRING))));
  ...
  public void fieldExposesMetadataDefaultAndAliases() {
    Field field = new Field("count", Schema.create(Type.INT), "item count", 3, Field.Order.IGNORE);
  ...
  public void fieldDistinguishesAbsentDefaultFromExplicitNullDefault() {
    ...
    Field explicitNull = new Field("explicitNull", nullable, null, Field.NULL_DEFAULT_VALUE);
  ...
  public void parserBuildsNamedRecordAndRetainsNamedTypes() {
    String json = "{\"type\":\"record\",\"name\":\"User\"," + "\"namespace\":\"example\",\"fields\":["
        + "{\"name\":\"id\",\"type\":\"long\"}]}";
  ...
  public void parserConcatenatesFragmentsAndRejectsDanglingContent() {
    Schema parsed = new Schema.Parser().parse("{\"type\":", "\"array\",", "\"items\":\"string\"}");
  ...
  public void parserLeavesInputStreamOpen() throws Exception {
    final class ObservableInputStream extends ByteArrayInputStream {
      ...
      ObservableInputStream(byte[] bytes) {
        super(bytes);
      }
    ...
    ObservableInputStream input = new ObservableInputStream("\"string\"".getBytes(StandardCharsets.UTF_8));
  ...
  public void parserDefaultValidationSettingControlsInvalidDefaults() {
    String json = "{\"type\":\"record\",\"name\":\"Configured\"," + "\"fields\":[{\"name\":\"count\",\"type\":\"int\","
        + "\"default\":\"bad\"}]}";
    ...
    Field count = parsed.getField("count");

    assertNotNull(parsed);
    assertNotNull(count);
    assertEquals(Type.INT, count.schema().getType());
  }
  ...
  public void applyAliasesRewritesRecordAndFieldNamesFromReaderAliases() {
    ...
    Schema reader = Schema.createRecord("User", null, "current", false, Collections.singletonList(readerField));
  ...
  public void jsonRenderingRoundTripsRepresentativeSchema() {
    Schema original = Schema.createRecord("Container", "doc", "example", false,
        Arrays.asList(new Field("names", Schema.createArray(Schema.create(Type.STRING))),
