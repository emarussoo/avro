...
public class ISW2SchemaC1LLMTest {
  ...
  public void recordFieldsAreAssignedPositionsAndLookedUpByName() {
    ...
    Schema record = Schema.createRecord(
        "Account", "account record", "example.avro", false, Arrays.asList(first, second));
  ...
  public void enumExposesSymbolsOrdinalsMembershipAndDefault() {
    Schema enumeration = Schema.createEnum(
        "Color", "available colors", "example.avro", Arrays.asList("RED", "GREEN"), "GREEN");
  ...
  public void inputStreamParsingLeavesStreamOpen() throws Exception {
    final class TrackingStream extends ByteArrayInputStream {
      ...
      TrackingStream(byte[] data) { super(data); }
  ...
  public void validDefaultChecksCoverScalarArrayMapAndRecordShapes() {
    ...
    Schema.Field optional = new Schema.Field(
        "note", Schema.createUnion(Schema.create(Schema.Type.NULL), Schema.create(Schema.Type.STRING)),
  ...
  public void jsonRenderingRoundTripsForRecursiveRecord() {
    ...
    Schema.Field next = new Schema.Field(
        "next", Schema.createUnion(Schema.create(Schema.Type.NULL), node),
        null, Schema.Field.NULL_DEFAULT_VALUE);
  ...
  public void applyAliasesRenamesWriterRecordAndFieldFromReaderAliases() {
    Schema writer = Schema.createRecord(
        "OldUser", null, "example.avro", false,
    ...
    Schema reader = Schema.createRecord(
        "User", null, "example.avro", false, Collections.singletonList(readerField));
