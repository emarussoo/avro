...
public class ISW2SchemaC2LLMTest {
  ...
  public void recordFactorySetsNamesDocumentationErrorFlagAndFields() {
    ...
    Schema record = Schema.createRecord(
        "User", "user document", "example.avro", true, Arrays.asList(id, active));
  ...
  public void enumExposesSymbolsOrdinalsMembershipAndDefault() {
    Schema schema = Schema.createEnum(
        "Color", "colors", "example", Arrays.asList("RED", "GREEN"), "GREEN");
  ...
  public void enumOrdinalRejectsUnknownSymbol() {
    Schema.createEnum("E", null, "example", Collections.singletonList("A"))
        .getEnumOrdinal("MISSING");
  }

  @Test
  public void unionPreservesBranchesIndexesNamesAndNullability() {
    Schema record = Schema.createRecord(
        "Item", null, "example", false, Collections.<Schema.Field>emptyList());
    Schema union = Schema.createUnion(
        Schema.create(Schema.Type.NULL), Schema.create(Schema.Type.STRING), record);
  ...
  public void fieldExposesDefaultOrderDocumentationAndAliases() {
    Schema.Field field = new Schema.Field(
        "count", Schema.create(Schema.Type.INT), "number of items", 7,
  ...
  public void nullDefaultIsDistinguishedFromAbsentDefault() {
    ...
    Schema.Field explicit = new Schema.Field(
        "explicit", nullSchema, null, Schema.Field.NULL_DEFAULT_VALUE);
  ...
  public void applyAliasesRewritesRecordAndFieldNamesFromReaderAliases() {
    Schema writer = Schema.createRecord("OldUser", null, "example", false,
        Collections.singletonList(
            new Schema.Field("oldId", Schema.create(Schema.Type.LONG))));

    Schema.Field readerField = new Schema.Field("id", Schema.create(Schema.Type.LONG));
    readerField.addAlias("oldId");
    Schema reader = Schema.createRecord("User", null, "example", false,
  ...
  public void structurallyEqualSchemasHaveEqualHashCodes() {
    List<Schema.Field> leftFields = Collections.singletonList(
        new Schema.Field("x", Schema.create(Schema.Type.INT)));
    List<Schema.Field> rightFields = Collections.singletonList(
        new Schema.Field("x", Schema.create(Schema.Type.INT)));
